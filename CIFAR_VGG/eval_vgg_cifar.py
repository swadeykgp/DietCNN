...
def forward_helper(item, x, index, patch_size, patch_stride, weights, n_clusters, centroid_lut, conv_lut, add_lut, bias_lut, fc_lut, relu_lut): 
    try:
        ...
        #print(x.shape)
        x = discretize(x,3, index,  patch_size, patch_stride,0)
        #print(x.shape)
        x = SymConv2D(x,3, 64, weights[0], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[0], 3, 2) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,64,  128, weights[1], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[1], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,128, 256, weights[2], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[2], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,256,  256, weights[3], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[3], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,256,  512, weights[4], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[4], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,512,  512, weights[5], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[5], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,512,  512, weights[6], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[6], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        x = SymConv2D(x,512,  512, weights[7], None, 3, 1, 1, # convolution parameters
            n_clusters, index, centroid_lut, patch_size, patch_stride, # patch params 
            conv_lut, add_lut, bias_lut[7], 3, 1) 
        #print(x.shape)
        x = SymReLU(x, relu_lut)
        #print(x.shape)
        #### Only conv
...
#        conv_lut, add_lut, bias_lut[0], 3, 2) 
#    end = time.time()
#    print("elapsed time for conv 0:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,64,  128, weights[1], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[1], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 1:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,128, 256, weights[2], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[2], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 2:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,256,  256, weights[3], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[3], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 3:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,256,  512, weights[4], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[4], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 4:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,512,  512, weights[5], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[5], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 5:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,512,  512, weights[6], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[6], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 6:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    start_t = time.time()
#    x = SymConv2D(x,512,  512, weights[7], None, 3, 1, 1, # convolution parameters
...
#        conv_lut, add_lut, bias_lut[7], 3, 1) 
#    end = time.time()
#    print("elapsed time for conv 7:", end - start_t)
#    #print(x.shape)
#    x = SymReLU(x, relu_lut)
#    #print(x.shape)
#    #### Only conv
...
class VGG_Sym(nn.Module):
    def __init__(self, sym_weights , sym_biases, n_clusters, index, centroid_lut, patch_size, patch_stride, conv_lut, fc_lut, add_lut, relu_lut):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            #nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(64, 128, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #18 #3
            nn.ReLU(inplace=True),
            #nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(128, 256, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #15 #6
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #13 #8
            nn.ReLU(inplace=True),
            #nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(256, 512, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #10 #11
            nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #8 #13
            nn.ReLU(inplace=True),
            #nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #5 #16
            nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(0, 0)), #3 #18
            nn.ReLU(inplace=True),
            #nn.MaxPool2d(kernel_size=2, stride=2),
            #nn.AvgPool2d(kernel_size=1, stride=1, padding=0),
        )
...
def vgg_sym(net,sd, filter_index_conv, filter_index_fc, conv_lut, fc_lut, add_lut, 
    ...
    f1_sym_filter = filter_to_sym_fc(filter_index_fc,f1_filter, patch_size, patch_stride, False)
    ...
    sym_weights.append(f1_sym_filter) 
  
    sym_biases = [] # These are layer 1 conv params
    sym_biases.append(c1_bias_lut)
    sym_biases.append(c2_bias_lut)
    sym_biases.append(c3_bias_lut)
    sym_biases.append(c4_bias_lut)
    sym_biases.append(c5_bias_lut)
    sym_biases.append(c6_bias_lut)
    sym_biases.append(c7_bias_lut)
    sym_biases.append(c8_bias_lut)
    sym_biases.append(f1_bias_lut)
    model = VGG_Sym(sym_weights , sym_biases, n_clusters, index, centroid_lut, patch_size, patch_stride, conv_lut, fc_lut, add_lut, relu_lut) 
...
if __name__ == '__main__':
    ...
    random_indices = list(range(0, len(testset), 100))
